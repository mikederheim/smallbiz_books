...
class JournalController extends Controller {
    ...
    public function delete(): void { check_csrf(); $cid=$this->companyId(); $id=(int)($_POST['id']??0); try{ $this->db->beginTransaction(); Ledger::deleteManualEntry($this->db,$cid,$id); AuditTrail::log($this->db,$cid,'deleted','journal_entry',$id,'Deleted manual journal entry'); $this->db->commit(); $this->redirect('journal'); }catch(Throwable $e){ if($this->db->inTransaction())$this->db->rollBack(); die($e->getMessage()); } }
    private function persist(bool $edit): void {
        ...
        try{ $this->db->beginTransaction(); if($edit) { Ledger::replaceEntry($this->db,$cid,$id,$date,$memo,$lines); AuditTrail::log($this->db,$cid,'updated','journal_entry',$id,'Updated manual journal entry'); } else { $id=Ledger::post($this->db,$cid,$date,$memo,$lines,'manual_journal',null); AuditTrail::log($this->db,$cid,'created','journal_entry',$id,'Created manual journal entry'); } $this->db->commit(); $this->redirect('journal'); } catch(Throwable $e){ if($this->db->inTransaction())$this->db->rollBack(); $this->formError($accounts,$entry,$posted,$e->getMessage()); }
    }
